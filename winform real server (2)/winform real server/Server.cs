...
namespace winform_real_server
{
    class Server
    {
        ...
        public void Proc_req_pi_ceid1(string[] receive_data_division, ref NetworkStream stream)
        {
            if (Pi.Ceid1_index != 1)
            {
                string send_data = "X";
            ...
            else if (Pi.Ceid1_index == 1)
            {
                if (Pi.Product_number.Substring(0, 1) == "\n")
                {
                    Pi.Product_number = Pi.Product_number.Substring(1, Pi.Product_number.Length - 1);
                    fm.textBox2.Text = "클라이언트에 보내는 값 = " + Pi.Product_number + "," + Pi.Model_name + "," + Pi.Prod_Percent + "," + Pi.Result + ","
                            + Pi.Fail_reason + "," + Pi.CV_move_state + "," + Pi.Robot_gripper_state;
                    string send_data = Pi.Product_number + "," + Pi.Model_name + "," + Pi.Prod_Percent + "," + Pi.Result + ","
                            + Pi.Fail_reason + "," + Pi.CV_move_state + "," + Pi.Robot_gripper_state;
                    byte[] Response_Data = Encoding.Default.GetBytes(send_data);
                    stream.Write(Response_Data, 0, Response_Data.Length);
                    Pi.Ceid1_index = 0;
        ...
        public void Server_Pi()
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("receive_data " + receive_data);
